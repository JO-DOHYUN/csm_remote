...
- Drive maps the post-deadband joystick linearly to `0..1000`, applies the existing time-equivalent slew and zero-before-reverse rule, and is configured at 200 Hz. Steering remains independently configured at 50 Hz. Co-scheduled drive/steering frames are drained into the CAN FIFO in one bounded service pass instead of retaining steering across the next 5 ms deadline. Unqualified/lost/failsafe RC emits only the drive stop frame when autonomy is explicitly released and hard/hardware gates are healthy. A 20 s PCAN/J4 capture measured `0x005` median 5.163 ms, p95 5.206 ms, max 6.168 ms with no interval above 7.5 ms; bus status, error/status frames, invalid payloads, runtime deadline misses, and CAN write failures were all zero.
