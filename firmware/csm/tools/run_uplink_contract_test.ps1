...
$compile = "call `"$vsDevCmd`" -no_logo -arch=x64 && cl /nologo /std:c++17 /EHsc /DCSM_TYPED_FRAME_NATIVE=1 /DBOARD_ENABLE_SERVICE_HIL_OBSERVABILITY=1 /DBOARD_UPLINK_POOL_MEDIUM_PAYLOAD_BYTES=192 /DBOARD_WIFI_SINK_QUEUE_RECORDS=256 /DBOARD_WIFI_SINK_QUEUE_BYTES=49152 /DBOARD_WIFI_SINK_CRITICAL_RESERVE_RECORDS=4 /DBOARD_WIFI_SINK_CRITICAL_RESERVE_BYTES=2112 /DBOARD_WIFI_TRANSIENT_COVERAGE_MS=0 /DBOARD_WIFI_PRESSURE_HIGH_WATER_BYTES=32768 /DBOARD_WIFI_PRESSURE_LOW_WATER_BYTES=8192 /DBOARD_WIFI_PRESSURE_HIGH_WATER_RECORDS=192 /DBOARD_WIFI_PRESSURE_LOW_WATER_RECORDS=64 /DBOARD_WIFI_STALL_TIMEOUT_MS=5000 /DBOARD_WIFI_CALL_STALL_TIMEOUT_MS=5000 /I`"$include`" $quotedSources /Fe:`"$output`""
