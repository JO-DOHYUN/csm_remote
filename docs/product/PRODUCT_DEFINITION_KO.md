...
- Service/HIL Host raw request는 static bus/ID/DLC/RTR allowlist와 위 공통 gate를
  통과한 뒤 sole `BuiltinCanTxOwner`를 통해 실제 3-slot FDCAN FIFO에 즉시 한 번
  제출한다. CSM은 허용 payload를 byte-preserve하며 차량 의미, cadence, count, ramp,
  CENTER, EHB 또는 neutral을 생성하지 않는다. HW busy/full은 현재 request를 reject하며
  Host SW backlog·retry가 없다.
