...
## 2026-08-03 transient-envelope gate

이 절이 아래 2026-07-27 queue/부하 수치를 대체한다. 현재 source candidate는
CAN0 2,000 fps + CAN1 2,000 fps, enabled stream exact 111,922 B/s,
qualification capacity 120,000 B/s, design envelope 135,000 B/s다.

검증 순서는 다음으로 고정한다.

1. C1: generated byte/record 계산, Wi-Fi `256/49,152`, USB `192/40,960`
   queue, critical reserve, 250 ms transient, pressure hysteresis, DTCM 64 KiB
   reserve, native contract와 exact product build를 닫는다.
2. I1a: 같은 Mbed/lwIP/WHD 설정의 lower-path capacity가 최소 120 kB/s,
   목표 135 kB/s를 지속하는지 별도 raw/product-identical gate로 측정한다.
3. I1b: exact product firmware에서 PC TCP consumer + Windows USB + dual CAN
   aggregate 4,000 fps를 동시에 실행한다. product stream은 111,922 B/s이므로
   socket rate 자체를 120 kB/s로 요구하지 않고 accepted 대비 drain ratio,
   queue 기울기와 loss 보존식으로 판정한다.
4. I2: I1b가 통과한 exact artifact를 Android Live/Capture, RC, CAN TX까지
   확장한다. PC 실패 전에는 Android 원인 분석을 시작하지 않는다.

I1b 기본 합격 경계는 injected source sequence exact, positive drain ratio 0.98
이상, 실제 injector active 구간의 complete diagnostic interval accepted rate
관측, end-start queue growth 2,920 B 이하, rejected/aborted/
socket-error/stall/reason-6 close 0, USB/Wi-Fi CRC·typed·segment·capture gap 0,
CAN source missing/duplicate/wrong-bus/drop 0이다. queue가 high-water에 들어간
사실만으로 실패나 reconnect로 판정하지 않는다. 실제 admission miss는 실패다.
accepted rate는 producer가 실제 제공한 양이므로 고정 floor로 lower-path
capacity를 주장하지 않는다. 120/135 kB/s는 I1a에서 별도로 생성·판정한다.

PC gate 명령은 Android workspace의 다음 도구가 권위다.

```powershell
python tools/hil_dual_can_stress.py --port COM7 --pcan-rate 2000 `
  --kvaser-rate 2000 --consume-wifi
```

현재 exact candidate는 C1과 PC I1b를 통과했다. 2026-08-03 final 60초 gate에서
PCAN 120,000과 Kvaser 120,000이 USB와 Wi-Fi 양쪽에서 exact였고, CRC/gap,
CAN/FIFO/pool/USB/Wi-Fi loss, close와 conservation residual은 모두 0이었다.
active accepted/drain은 100,190/100,207 B/s, queue change는 -981 B였다.
I1a 120/135 kB/s capacity와
Android+Capture/RC/CAN-TX I2는 OPEN이다.
